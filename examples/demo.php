...
if(isset($_GET['title']) && !empty($_GET['title'])) {
	...
	*/
	...
	$query->createFilterQuery('titleFiler')->setQuery('title:*' .$title.'*' );
...
?>
		...
		<div style="float:left;margin-left:30px;">
			...
			if ($start !== 0) {
				echo '<a href="?title=' . $title. '&start='.($start-$query->getRows()).'">&laquo; previous</a>';
				echo '&nbsp;&nbsp;&nbsp';
			}
			...
			if ($result->getNumFound() > ($start+$query->getRows())) {
				echo ' <a href="?title=' . $title. '&start='.($start+$query->getRows()).'">next &raquo;</a>';
			}
